...
function currentHintMatches(word, hintQueue, hintIndex) {
  ...
  const hint = hintQueue[hintIndex - 1];
...
test('escapes <>&" characters', () => {
  // Note: textContent/innerHTML only escapes <, >, & (not " which is safe in text nodes)
...
test('"ba" is too short for "BA.. 5" hint', () => {
  assert.strictEqual(currentHintMatches('ba', ['BA.. 5'], 1), false);
});

test('exact boundary match: word length matches hint exactly', () => {
  assert.strictEqual(currentHintMatches('baker', ['BA.. 5'], 1), true);
});

test('null word returns false', () => {
  assert.strictEqual(currentHintMatches(null, ['BA.. 5'], 1), false);
});

test('undefined word returns false', () => {
  assert.strictEqual(currentHintMatches(undefined, ['BA.. 5'], 1), false);
});

test('single-hint queue with matching word "xyz" at index 1', () => {
  assert.strictEqual(currentHintMatches('xyz', ['XY.. 3'], 1), true);
});

...
3. Stage the changed files and commit with subject `v<new_version>: <task_summary>` (matches the repo convention). The ship-guard hook blocks only if the @version wasn't bumped (or if a command targets `main`). If blocked, report the hook message and stop.
